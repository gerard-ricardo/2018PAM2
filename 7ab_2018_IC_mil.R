...
str(data1)
data1$disc1 <- as.factor(as.character(data1$disc1))
#data1 <- subset(data1, dli!= 'r21')
data1 <- data1 %>% filter(!(dli == 0.88 & spec == "whi"))
# ########fvfm###################
...
data.y3.long <- data.y3.long[complete.cases(data.y3.long), ] %>%
  dplyr::arrange(disc1) %>%
  data.frame()
...
split_df = split(df6, df6$spec)
whi_df = split_df$whi
yel_df = split_df$yel
ECx::ecx_interp(ecx = 0.1,  x = whi_df$raw.x, pred = whi_df$prediction, lower = whi_df$lower, upper = whi_df$upper)
ecx_interp(ecx = 0.1,  x = yel_df$raw.x, pred = yel_df$prediction, lower = yel_df$lower, upper = yel_df$upper)

#bayesnec ECx
ecx(exp_1, ecx_val = 10,  xform = exp)
ecx(exp_2, ecx_val = 10,  xform = exp)

###
result <- df6 %>%
  group_by(spec) %>%
  summarise(
    prediction_at_min_x = prediction[which.min(x)],
    prediction_at_max_x = prediction[which.max(x)]
  ) %>%  data.frame()
result$diff_prop <- ((result$prediction_at_max_x - result$prediction_at_min_x)/ result$prediction_at_min_x) * 100
result
